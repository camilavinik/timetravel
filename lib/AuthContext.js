...
import { supabase, provisionalClient } from './supabase'
...
export const AuthProvider = ({ children }) => {
  ...
  const changePassword = async ({ currentPassword, newPassword }) => {
    ...
    try {
      ...
      const { error: signInError } = await provisionalClient.auth.signInWithPassword({
      ...
      // Update password
      const { error: updateError } = await provisionalClient.auth.updateUser({ password: newPassword })
      if (updateError) {
      ...
      // Close the provisional client after the operation is complete
      provisionalClient.removeAllChannels()
    }
  }

  return (
    <AuthContext.Provider value={{ session, profile, loading, register, logout, login, changePassword }}>
      {children}
